...
class WaypointRecorder(Node):
    ...
    def save_to_file(self):
        """Append new waypoints to existing waypoint_list.py instead of overwriting"""
        ...
        if os.path.exists(file_path):
            try:
                ...
                spec = util.spec_from_file_location("waypoint_list", file_path)
                module = util.module_from_spec(spec)
                spec.loader.exec_module(module)

                wp = module.WayPoints()
                existing_trackin = wp.get_trackin()
                existing_trackout = wp.get_trackout()
            except Exception as e:
                print(f"Warning: failed to load existing waypoints ({e}). Starting fresh.")
        ...
        merged_trackin = merge_unique(existing_trackin, self.trackin_waypoints)
        merged_trackout = merge_unique(existing_trackout, self.trackout_waypoints)
...
'''
        if merged_trackin:
            for i, wp in enumerate(merged_trackin):
...
'''
        if merged_trackout:
            for i, wp in enumerate(merged_trackout):
                content += f"            {wp},  # waypoint {i}\n"
        else:
            content += "            # empty\n"
...
'''
        ...
        try:
            ...
            print("\n" + "="*70)
            print("Waypoints appended and saved successfully!")
            print(f"File: {file_path}")
            print(f"Track IN:  {len(merged_trackin)} total waypoints")
            print(f"Track OUT: {len(merged_trackout)} total waypoints")
            print("="*70 + "\n")
